...
public class ReadMotif implements Action {
    ...
    public ReadMotif(Telemetry telemetry) {
    ...
    public boolean run(@NonNull TelemetryPacket packet) {
        if (Limelight.limelight.getLatestResult().isValid()) {
            packet.put("limelight fiducial id", Limelight.limelight.getLatestResult().getFiducialResults().get(0));
            telemetry.addData("limelight fiducial id", Limelight.limelight.getLatestResult().getFiducialResults().get(0));
        }
